    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <Field label={t('대상 Key Result', 'Target Key Result')}>
            <select className="input" value={krId} onChange={(e) => setKrId(e.target.value)}>
              <option value="">{t('KR을 선택하세요', 'Select a KR')}</option>
          ...
          {krId && (
            <div className="mt-3 space-y-3">
              {([
                ['progress_summary', t('이번 주 진행 내용', "This week's progress")],
                ['completed_work', t('완료한 것', 'Completed')],
                ['blockers', t('막힌 것 (blocker)', 'Blockers')],
                ['next_week_actions', t('다음 주 액션', 'Next week actions')],
                ['support_needed', t('필요한 지원', 'Support needed')],
              ...
              <button className="btn-primary w-full" disabled={submit.isPending} onClick={() => submit.mutate()}>
                {submit.isPending ? t('제출 중…', 'Submitting…') : t('CFR 제출 + AI 분석', 'Submit CFR + AI analysis')}
              </button>
            </div>
          )}
          {ai && <div className="mt-4"><AiResultCard result={ai} title={t('CFR AI 분석', 'CFR AI analysis')} /></div>}
        </Card>

        <Card>
          <h3 className="mb-2 text-sm font-semibold text-slate-700 dark:text-slate-200">{t('내 CFR 히스토리', 'My CFR history')}</h3>
          {history.isLoading ? <Spinner /> : (
            <div className="space-y-2">
              ...
              {(history.data ?? []).length === 0 && <p className="text-sm text-slate-400 dark:text-slate-500">{t('아직 작성한 CFR이 없습니다.', 'No CFR check-ins yet.')}</p>}
            </div>
          )}
        </Card>
      </div>
