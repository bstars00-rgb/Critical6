  ...
  return (
    <>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <Card>
          <h3 className="mb-3 text-sm font-semibold text-slate-700 dark:text-slate-200">회사 Objective 진행 현황</h3>
          <div className="space-y-3">
            {s.companyObjectives.length === 0 && <p className="text-sm text-slate-400 dark:text-slate-500">데이터 없음</p>}
            {s.companyObjectives.map((o: any) => (
              <Link key={o.id} to={`/okr/${o.id}`} className="block">
                <div className="mb-1 flex items-center justify-between text-sm">
                  <span className="font-medium text-slate-700 dark:text-slate-200">목표 #{o.id.slice(0, 4)}</span>
        ...
        <Card>
          <h3 className="mb-3 text-sm font-semibold text-slate-700 dark:text-slate-200">KPI 달성률</h3>
          <div className="space-y-2">
            ...
            {(kpis.data ?? []).length === 0 && <p className="text-sm text-slate-400 dark:text-slate-500">KPI 없음</p>}
          </div>
        </Card>
      </div>

      <Card className="mt-4">
        <h3 className="mb-3 text-sm font-semibold text-slate-700 dark:text-slate-200">AI 위험 알림</h3>
        <div className="space-y-2">
          ...
          {(insights.data ?? []).length === 0 && <p className="text-sm text-slate-400 dark:text-slate-500">활성 알림 없음</p>}
        </div>
      </Card>
    </>
  );
