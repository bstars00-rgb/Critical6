...
export default function OkrDetail() {
  ...
  const d = data.data!;
  if (!d.objective) return <p className="text-slate-400">목표를 찾을 수 없습니다.</p>;
  const o = d.objective;

  return (
    <>
      <Link to="/okr" className="mb-2 inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card><div className="text-xs text-slate-500">진행률</div>
          <div className="mt-1 text-2xl font-bold">{Math.round(o.progress)}%</div>
          <ProgressBar value={o.progress} className="mt-2" /></Card>
        <Card><div className="text-xs text-slate-500">상태</div><div className="mt-2"><StatusBadge status={o.status} /></div></Card>
        <Card><div className="text-xs text-slate-500">우선순위</div><div className="mt-2"><PriorityBadge priority={o.priority} /></div></Card>
      </div>
      ...
      <Card className="mt-4">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-700">Key Results ({d.keyResults.length})</h3>
          <button className="btn-ghost" onClick={() => setKrModal(true)}><Plus className="h-4 w-4" />KR 추가</button>
        </div>
        <div className="space-y-2">
          {d.keyResults.length === 0 && <p className="text-sm text-slate-400">KR이 없습니다.</p>}
          {d.keyResults.map((kr: any) => (
            <div key={kr.id} className="rounded-lg border border-slate-100 p-3">
              <div className="flex items-center gap-2">
                <span className="flex-1 text-sm font-medium text-slate-700">{kr.title}</span>
                <select className="rounded border border-slate-200 px-2 py-1 text-xs"
                  ...
                  {OKR_STATUSES.map((s) => <option key={s} value={s}>{STATUS_LABEL[s]}</option>)}
                </select>
              </div>
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className="w-10 text-right text-xs text-slate-500">{Math.round(kr.progress)}%</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <Card>
          <h3 className="mb-2 text-sm font-semibold text-slate-700">연결된 KPI ({d.kpis.length})</h3>
          {d.kpis.map((k: any) => (
            <div key={k.id} className="flex justify-between py-1 text-sm">
              <span className="text-slate-600">{k.name}</span>
              <span className="text-slate-400">{k.current_value ?? '—'}/{k.target_value ?? '—'} {k.unit}</span>
            </div>
          ))}
          {d.kpis.length === 0 && <p className="text-sm text-slate-400">없음</p>}
        </Card>
        <Card>
          <h3 className="mb-2 text-sm font-semibold text-slate-700">Critical 6 / Action ({d.criticalSix.length + d.actionPlans.length})</h3>
          {[...d.criticalSix, ...d.actionPlans].map((t: any) => (
            <div key={t.id} className="flex justify-between py-1 text-sm">
              <span className="text-slate-600">{t.title}</span><StatusBadge status={t.status} />
            </div>
          ))}
          {d.criticalSix.length + d.actionPlans.length === 0 && <p className="text-sm text-slate-400">없음</p>}
        </Card>
      </div>
