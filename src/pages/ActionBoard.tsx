...
import { BOARD_COLUMNS, STATUS_LABEL, STATUS_COLOR } from '@/lib/constants';
...
export default function ActionBoard() {
  ...
  return (
    <>
      <PageHeader title="Action Board" subtitle="모든 Action은 가능하면 OKR/KR/Critical 6에 연결"
        action={<button className="btn-primary" onClick={() => setModal(true)}><Plus className="h-4 w-4" />작업</button>} />
          ...
          return (
            <div key={col} className="rounded-xl bg-slate-100/60 p-2">
              <div className="mb-2 flex items-center justify-between px-1">
                <span className={cn('rounded-full px-2 py-0.5 text-xs font-medium', STATUS_COLOR[col])}>{STATUS_LABEL[col]}</span>
                  ...
                  return (
                    <div key={a.id} className="card p-2">
                      ...
                      <div className="mt-1.5 flex items-center gap-1.5">
                        ...
                        {a.due_date && <span className="ml-auto text-[10px] text-slate-400">{a.due_date}</span>}
                      </div>
                      <select className="mt-1.5 w-full rounded border border-slate-200 px-1 py-0.5 text-xs"
                        ...
                        {BOARD_COLUMNS.concat('cancelled').map((s) => <option key={s} value={s}>{STATUS_LABEL[s]}</option>)}
                      </select>
                    </div>
                  );
                })}
